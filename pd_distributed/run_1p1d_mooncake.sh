#!/bin/bash
# =============================================================================
# 1P1D Disaggregated Serving Demo — MooncakeConnector (TCP)
# =============================================================================
# GPU0 = Prefill (kv_producer), GPU1 = Decode (kv_consumer)
# Mooncake TransferEngine 使用 TCP 协议进行 KV cache 传输
# Proxy 负责将请求分发到 Prefill / Decode 实例
# =============================================================================
...
LOG_DIR="${SCRIPT_DIR}/logs_mooncake"
mkdir -p "${LOG_DIR}"
...
echo "============================================================"
echo " 1P1D MooncakeConnector Demo (TCP)"
echo "============================================================"
echo " Model:          ${MODEL}"
...
python -m vllm.entrypoints.openai.api_server \
    ...
    --trust-remote-code \
    --kv-transfer-config \
    '{"kv_connector":"MooncakeConnector","kv_role":"kv_producer","kv_connector_extra_config":{"mooncake_protocol":"tcp"}}' \
...
# ---- Proxy ----
echo "[launch] Starting Mooncake proxy (port ${PROXY_PORT})..."
